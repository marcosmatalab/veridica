...
- **Conjunto:** oro corregido — **94 pares (19 `busqueda`, 75 `lectura`)**; el diff del propietario
  aplicado en el commit `2e0dbdc`, `verificar_oro` en verde **antes de cada corrida** (regla del
  3.5)
- **Corridas:** `corridas_eval` ids **20-25**, con el arnés commiteado (`98e9899`). Las ids 14-19
  fueron la primera pasada con el mismo árbol antes de commitear el arnés; la re-corrida reprodujo
  **métricas idénticas** en las seis (comprobado con un `=` de jsonb, quitando solo el tiempo por
  consulta).
...
|---|---|---:|---:|---:|---:|---:|
| Léxica (3.1) | 20 | 39,0 % | **27,7 %** | 22,7 % | 61,0 % | **48,9 %** |
| Vectorial (3.2) | 21 | 73,0 % | **60,6 %** | 58,7 % | 82,0 % | **76,6 %** |
...
| Vectorial | 0,446 | 0,557 | 0,418 |
| Fusión 10:1, orden de la fusión | 0,411 | 0,484 | 0,393 |
| Fusión 10:1 + reordenador (top 6) | 0,410 | 0,531 | 0,379 |
...
conjunto corregido (corridas 23 y 24), en `lectura`:
...
(68,4 % contra 57,9 %); el `nDCG@5` global queda en tablas (0,410 contra 0,411). La mejora del
...
| dos métricas partidas por `localizacion` además del global | ✓ `recall@6` y `nDCG@5`, en §1 y en `corridas_eval` 20-25 |
