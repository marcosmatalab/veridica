...
**Y ESA MIRADA SE HACE CON RECARGA FORZADA, porque la puerta que este proyecto pone por encima de los tests también tiene su verde mentiroso, y el suyo es la caché del navegador.** Pasó el 12 de agosto de 2026: la primera captura tras un arreglo de la hoja era la hoja **cacheada**, y habría dado un veredicto —bueno o malo, da igual— sobre una página que ya no existía. Es la avería de siempre con otro disfraz: el instrumento mintiendo, no lo medido, y aquí el instrumento es el ojo. El mecanismo, para que la nota no dependa de recordarlo: la hoja se sirve **sin `Cache-Control`**, solo con `ETag` y `Last-Modified`, así que el navegador decide por su cuenta cuánto tiempo la da por buena y puede servirla de caché **sin preguntar al servidor**. Así que: recarga forzada antes de mirar, o comprobar antes que `/estatico/estilo.css` está sirviendo lo que se acaba de escribir. Vale igual para cualquier captura que se guarde como evidencia.
...
**Y el ensayo y la grabación empiezan con una recarga forzada del navegador**, por lo que dice el 2.4: la hoja se sirve sin `Cache-Control`, así que una pantalla que se acaba de tocar puede seguir enseñando la versión anterior sin avisar. Grabar la evidencia de una página cacheada es peor que no grabarla, porque queda en el repo con aspecto de prueba.
