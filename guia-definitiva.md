...
- **UN SELECTOR QUE NO CASA CON LO QUE CREES REPORTA ÉXITO IGUAL: un verde sobre una selección vacía o equivocada es indistinguible de un verde sobre lo que querías mirar.** Familia del instrumento que miente, y **la octava**. El caso, del 14 de agosto de 2026, en mitad del ritual de la mutación: con `web/index.html` **mutado a propósito** a la cabecera falsa, `pytest -k "cartel"` contestó **«1 passed»** —y era verdad, solo que de OTRO test: el que perseguía se llama `no_declara_capacidades` y no lleva la palabra dentro, así que la selección lo dejó fuera. Un segundo más y el arreglo se declara validado por una sonda que nunca corrió. **La comprobación cuesta leer una línea que ya sale en pantalla: mirar el RECUENTO de recogidos** —`1 passed, 42 deselected` no es el mismo hecho que `7 passed, 36 deselected`— **y exigir que cuadre con cuántos tests creías tener** antes de creerse el resultado. **Y va pegado su gemelo del mismo minuto**, porque el par enseña más que cada uno: `curl | grep "Encargo 2.4"` devolvía **2 después** del arreglo, y no porque el arreglo fallara, sino porque los comentarios del propio fichero **citan las cadenas viejas** para contar por qué se quitaron. **El grep miraba el cuerpo servido; el alumno mira la página renderizada: dos poblaciones distintas para la misma pregunta**, que es la regla de la población mal elegida cometida dentro de una comprobación de tres segundos. En los dos, el aparato contestó con precisión a una pregunta que no era la que se le hacía.
