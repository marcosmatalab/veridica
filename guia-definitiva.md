...
**1.3 Normalización.** Todo a markdown o texto limpio (conversión de PDF y HTML con revisión por muestreo: 10 documentos al azar leídos a ojo). Verificación: cero binarios en el árbol de texto; muestreo anotado en el ADR de la fase. **Decisión pendiente que se resuelve AQUÍ, planteada desde el encargo 1.0 y no antes:** este encargo genera ficheros nuevos (el markdown convertido) y el manifiesto crece con ellos, así que hay que decidir con qué forma entran. Las opciones sobre la mesa: (a) entrada propia por fichero convertido con un campo que apunte a su original (`derivado_de`) y el original conservado con su hash, que hace la cadena PDF→texto auditable y duplica entradas; (b) el convertido sustituye al original en el manifiesto, más limpio y sin trazabilidad de qué se convirtió desde qué; (c) el hash del convertido viaja dentro de la entrada del original, sin entrada propia, que ahorra entradas y rompe la regla de "un fichero en disco, una entrada". La decisión va con ADR, y el verificador del 1.0 ya está preparado para cualquiera de las tres: tolera campos nuevos en las entradas y no supone que el manifiesto sea inmutable.
