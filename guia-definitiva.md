...
**Aun así, la mirada sigue haciéndose con recarga forzada**, porque la cabecera arregla el mecanismo y no la situación: una pestaña ya abierta enseña lo que cargó cuando cargó y nada la refresca sola, y una copia guardada **antes** de que existiera la cabecera conserva las reglas de frescura con las que se guardó. Recargar es gratis; un veredicto sobre una página que ya no existe, no. Vale igual para cualquier captura que se guarde como evidencia.
...
**Y el ensayo y la grabación empiezan con una recarga forzada del navegador.** `/estatico` ya manda revalidar (`Cache-Control: no-cache`, ver 2.4), así que el mecanismo está arreglado; lo que no arregla ninguna cabecera es que una pestaña abierta desde hace media hora sigue enseñando lo que cargó entonces. Grabar la evidencia de una página cacheada es peor que no grabarla, porque queda en el repo con aspecto de prueba.
