...
- **UN CASO NEGATIVO INVENTADO A OJO NO ES EL CASO: ES LO QUE UNO CREE QUE ERA EL CASO.** Toda sonda se valida contra un caso donde debe fallar —eso ya está escrito—, pero **el caso tiene que ser el fallo REAL, recuperado, no una reconstrucción de memoria**. El caso, del 15 de agosto de 2026: el `gantt` del README no renderizaba en GitHub, y para probar que la puerta nueva lo cazaba escribí *un gantt mínimo con la misma pinta*. **Renderizaba perfectamente**, así que la validación "en la dirección contraria" no validaba nada y la puerta habría quedado publicada como probada sin haber visto jamás un fallo. Hubo que sacar el original de `git show HEAD:README.md` para tener el caso de verdad — y entonces sí salió, palabra por palabra, el mismo `TypeError: Cannot read properties of undefined (reading 'type')` que enseñaba GitHub. **Lo que uno reconstruye de memoria es su HIPÓTESIS sobre el fallo, y si la hipótesis fuera correcta la sonda ya no haría falta.** La comprobación cuesta un comando y siempre está a mano, porque el fallo real casi siempre sigue existiendo en algún sitio: `git show`, la corrida guardada, el log del CI, la fila de la base. **Y hay un segundo modo del mismo error, que también cayó ese día: el rojo por el motivo equivocado.** El primer intento de esa misma validación dio rojo por una excepción de rutas dentro del propio script —mismo código de salida, motivo distinto—, o sea una sonda dándose por validada por su propia avería. Así que no basta con que la sonda se ponga roja: **hay que leer POR QUÉ se puso roja**.
